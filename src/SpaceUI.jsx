   ...
   <div style={{flex:1,paddingLeft:isMob?"0":"clamp(20px,3vw,40px)",paddingTop:"8px",minWidth:0,overflow:"hidden"}}>
    ...
    {tab==="report"&&(()=>{
      ...
      return (
        <div style={{maxWidth:"480px",display:"flex",flexDirection:"column",gap:"20px"}}>
          <div>
            <div style={{fontSize:"1.05rem",fontWeight:"700",color:"var(--text-primary)",fontFamily:F2,marginBottom:"4px"}}>📱 Daily Report</div>
          ...
          <div>
            ...
            {recipients.map(email => (
              <div key={email} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",background:"var(--bg-1)",border:"1px solid var(--border-1)",borderRadius:"10px",marginBottom:"6px"}}>
                <span style={{fontSize:"0.88rem",fontWeight:"600",color:"var(--text-primary)",fontFamily:F2}}>{email}</span>
                <button onClick={()=>removeRecipient(email)} style={{background:"none",border:"none",color:"rgba(248,113,113,0.7)",cursor:"pointer",fontSize:"0.8rem",fontWeight:"700",padding:"2px 6px",fontFamily:F2}}>Remove</button>
              </div>
            ))}
          </div>
          ...
          <div>
            ...
            <div style={{display:"flex",gap:"8px"}}>
              <input type="email" placeholder="austin@inmangroup.net" value={newEmail}
                ...
                style={{flex:1,background:"var(--bg-2)",border:"1px solid var(--border-1)",borderRadius:"10px",padding:"10px 14px",color:"var(--text-primary)",fontSize:"0.88rem",fontFamily:F2,outline:"none"}}
              />
              <button onClick={addRecipient} disabled={saving||!isValidEmail(newEmail)}
                style={{background:"var(--accent)",color:"#000",border:"none",borderRadius:"10px",padding:"10px 16px",fontWeight:"800",fontSize:"0.85rem",cursor:"pointer",fontFamily:F2,opacity:(!isValidEmail(newEmail)||saving)?0.4:1}}>
                {saving?"…":"Add"}
              </button>
            </div>
          </div>
          ...
          <div style={{background:"rgba(245,158,11,0.06)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:"12px",padding:"14px 16px"}}>
            ...
            <div style={{fontSize:"0.75rem",color:TM,lineHeight:1.7}}>
              Deploy the <code style={{color:TA}}>daily-report</code> edge function, then add a cron in Supabase:<br/>
          ...
          {recipients.length > 0 && (
            <button onClick={sendTest} disabled={testSending}
              style={{background:"rgba(29,201,232,0.08)",border:"1px solid rgba(29,201,232,0.25)",borderRadius:"12px",padding:"13px",fontWeight:"700",fontSize:"0.88rem",color:TA,cursor:"pointer",fontFamily:F2,opacity:testSending?0.6:1}}>
